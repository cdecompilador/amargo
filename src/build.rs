...
macro_rules! direct_dependencies {
    ($path:expr, $dep_exts:expr, $deps:expr) => {{
        ...
        // very well (and they are mandatory unless I use a full custom c parser)
...
impl<'a> Build<'a> {
    ...
    pub fn new(config: &'a ProjectConfig, mode: BuildType) -> Result<Build<'a>, Error> {
        ...
        let target_path = build.out_dir.join(project_name)
    ...
    pub fn link(&mut self) -> Result<bool, Error> {
        ...
        let target_path = self.out_dir.join(project_name)
            .with_extension(EXE_EXTENSION);

        // If the executable exist and its up to date do not recompile
        if target_path.is_file() {
            let target_path_modif = target_path.metadata().unwrap().modified().unwrap();
