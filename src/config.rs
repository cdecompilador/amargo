...
pub(crate) struct Cli {
    /// Subcommands of `amargo` here is the important stuff like `run`, `build, `new`.
    #[clap(subcommand)]
...
/// All the configs needed of the project to execute any subcommand in `amargo` or
/// call the build AP
...
pub enum ProjectType {
    ...
    /// Library project with a entry lib.c that will compile to a <project_name>.h
    /// and a <project_name>.a/lib
    #[display("library (static)")]
    #[clap(name="static")]
    StaticLib,

    /// Library project with a entry lib.c that will compile to a <project_name>.h
    /// and a <project_name>.so/dll
...
pub(crate) enum Command {
    ...
    Run {
        ...
        /// The arguments provided in the form `-- <exe_args..>` they are passed as
        /// arguments to the target to run (if any)
        #[clap(last = true)]
        exe_args: Vec<String>,
    },
